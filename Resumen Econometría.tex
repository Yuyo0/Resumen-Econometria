...
var($u_i|x_i$) = $\sigma^2_i$ es una constante. Si este supuesto adicional se cumple, el estimador MCO es eficiente
...
Bajo todos estos supuestos $u_i$ es normal con media en cero y varianza $\sigma^2_i$ y la distribución condicional
de $u_i|x_i$ es normal con media en cero y varianza en var($u_i|x_i$) = $\sigma^2_i$, la cual es $\sigma^2_i$ por el cuarto supuesto.
...
Se deduce de los resultados anteriores que el primer término seguido a la igualdad converge en distribución a N(0,1), 
...
$$\sigma^2_{\beta_1|X}=\frac{\sigma_u^2}{\sum_{i=1}^{n}(X_i-\bar{X})^2}$$

Para verificar este debe establecerse primero que la distribución es normal
, para esto debe tenerse en cuenta que condicionado a $x_i$, $\hat{\beta}_1-\beta_1$ 
es una media ponderada de los errores $u_i$:
$$\frac{1}{-}\sum_{i=1}^{n}(X_{i}-\bar{X})u_{i}\widehat{\beta}_1=\beta_1+\frac{n_i\sum_{i=1}^n(X_i-\bar{X})u_i}{\frac{1}{n}\sum_{i=1}^n(X_i-\bar{X})^2}$$
...
La ventaja de utilizar errores estándar heterocedástico-robustos es que dan lugar a inferencias asin-
tóticamente válidas incluso si no se conoce la forma de la función de la varianza condicional. Una ventaja
...
adicional para protegerse frente a esa amenaza. La desventaja de los errores estándar heterocedástico-
robustos consiste en que el estimador MCO tendrá una mayor varianza que el estimador MCP (basado
