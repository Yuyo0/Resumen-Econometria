...
\newtcolorbox{conceptoclave}[1]{
  enhanced,
  colback=stockwatsonblue!5!white,
  colframe=stockwatsonblue,        
  coltitle=white,
  fonttitle=\sffamily\bfseries,
  % --- CONFIGURACIÓN DE TÍTULO MULTILÍNEA ---
  varwidth boxed title,
  before title={\hypersetup{linkcolor=white, citecolor=white, urlcolor=white}},
  title={#1}, 
  attach boxed title to top left={yshift*=-\tcboxedtitleheight/2, xshift=5mm},
  boxed title style={
    colback=stockwatsonblue, 
    sharp corners=south,
    size=small,
...
investigar un efecto causal. Los datos observacionales (más habituales) se obtienen mediante la observación 
del comportamiento real fuera de un marco experimental. Estos se recopilan utilizando encuestas
...
periodo de tiempo.
    \item \textbf{Datos de series temporales:} son datos para un único individuo o entidad recogidos para múltiples 
    periodos. El número de observaciones en estos casos se expresa como observaciones. Estos
...
La \textbf{Ley de esperanzas iteradas:} La media de $Y$ es la media ponderada de la esperanza condicional de $Y$ dado $X$,
ponderada por la distribución de probabilidad de $X$, si $X$ toma $l$ valores $x_1,...,x_l$, entonces:
...
a una distancia menor o igual a $\delta$ de $\mu$ tiende a cero a medida que n tiende a infinito, siendo $\delta$ positivo. Es
decir:
...
\subsection*{Apéndice 17.2: Dos desigualdades}

\subsubsection*{La desiguldad de Chebychev}
La desigualdad de Chebychev utiliza la varianza de la variable aleatoria $V$ con el fin de acotar la probabilidad de que
$V$ se encuentre a más distancia que $\pm \delta$ respecto de su media, donde $\delta$ es una constante positiva:
$$\Pr(|V-\mu_V|\geqslant\delta)\leqslant\frac{\operatorname{var}(V)}{\delta^2}$$
...
E(W^{2}) & =\int_{-\infty}^{\infty}w^{2}f(w)dw \\
 ...
 & \geqslant\int_{-\infty}^{-\delta}w^{2}f(w)dw+\int_{\delta}^{\infty}w^{2}f(w)dw \\
 & \geqslant\delta^{2}\left[\int_{-\infty}^{-\delta}f(w)dw+\int_{\delta}^{\infty}f(w)dw\right] \\
 & =\delta^{2}\Pr(|W|\geqslant\delta),
\end{align}

La primera desigualdad se cumple debido a que el término que no es tenido en cuenta es no
negativo, la segunda desigualdad se cumple debido a que $w^2 \geqslant \delta^2$ a lo largo de todo el rango de integración y la última
igualdad se cumple por la definición de $Pr(\mid W\mid \geqslant \delta)$. ustituyendo $W = V -\mu_V$ en la última expresión, teniendo en
cuenta que $E(W^2) = E[(V -\mu_V)^2] = var(V)$, y reordenando se obtiene la desigualdad. Si $V$ es discreta, esta prueba es aplicable con sumatorios en sustitución de las integrales.

\subsubsection*{La desigualdad de Cauchy-Schwarz}

La desigualdad de Cauchy-Schwarz es una generalización de la desigualdad de la correlación $\mid \rho_{XY}\mid \leqslant 1$, para 
incorporar medias distintas de cero. La desigualdad de Cauchy-Schwarz es:
$$|E(XY)|\leqslant\sqrt{E(X^2)E(Y^2)}$$

La demostración es similar a la prueba de la desigualdad de la correlación del Apéndice 2.1. Sea
$W = Y + bX$, donde b es una constante. Entonces $E(W^2) = E(Y^2) + 2bE(XY) + b^2E(X^2)$. Ahora sea 
$b =-E(XY)/E(X^2)$ por lo que (después de la simplificación) la expresión se convierte en 
$E(W^2)=E(Y^2)- [E(XY)]^2/E(X^2)$. Debido a que $E(W^2) = 0$ (debido a que $W^2 \geqslant 0$), debe ocurrir que 
$[E(XY)]^2 = E(X^2)E(Y^2)$, y la desigualdad de Cauchy-Schwarz se deduce tomando la raíz cuadrada.
...
Sea $\rho_{xu}$ la correlación entre $X_i$ y $u_i$, y que se cumplen el 2do y 3er supuesto de MCO, entonces el
...
$$Expresion \quad \beta_1= \frac{ \Delta Y}{ \Delta X_1} \quad manteniendo \quad X_2 \quad constante$$
...
Los coeficientes que minimizan esto son los estimadores de mínimos cuadrados de $b_0, b_1, . . . , b_k$. Las
...
$$ ESR=s_{\hat{u}}\quad \text{donde}\quad s_{\hat{u}}^2=\frac{1}{n-k-1} \sum_{i=1}^n \hat{u}_i^2=\frac{S R}{n-k-1}$$
...
distribución normal bivariante. Para el análisis de regresión múltiple, esto se hace extensible para la distribución 
normal multivariante. El teorema central del límite se aplica del mismo modo, los estimadores
...
La multicolinealidad imperfecta implica que dos o más de los regresores están altamente correlacionados. 
Esto no resulta un problema para la teoría de los MCO. Si los regresores presentan multicolinealidad
...
\subsection*{Apéndice 6.1: Obtención de la ecuación 6.1 (\ref{eq:6.1})}

Se parte de la ecuación:
$$\hat{\beta}_1=\beta_1+\frac{\frac{1}{n} \sum_{i=1}^n\left(X_i-\bar{X}\right) u_i}{\frac{1}{n} \sum_{i=1}^n\left(X_i-\bar{X}\right)^2}$$

Bajo los 2 últimos supuestos del concepto clave 4.3 (\ref{cc:4.3}). $(1/n)\sum_{i=1}^{n} (X_i-\bar{X})^2 \xrightarrow{p} \sigma_X^2$, 
y $(1/n)\sum_{i=1}^{n}(X_i-\bar{X})u_i \xrightarrow{p} cov(u_i,X_i)=\rho_{Xu}\sigma_u\sigma_X$. Mediante la sustitución de estos límites en la Ecuación 
recién presentada se llega a la ecuación 6.1 (\ref{eq:6.1}).   

\subsection*{Apéndice 6.2: Distribución de los estimadores MCO en presencia de dos regresores y errores homocedásticos}
...
\subsection*{Apéndice 6.3: El teorema de Frisch-Waugh}
...
de las medias muestrales por las esperanzas, lo que conducía a un estimador $\hat{\sigma}^2_{\hat{\beta}_1}$. Bajo mínimos cuadrados,
la ley de grandes números implica que las medias muestrales convergen a las poblacionales, siendo que $\hat{\sigma}^2_{\hat{\beta}_1}/\sigma^2_{\hat{\beta}_1}$ 
converge en probabilidad a $1$. La raíz cuadrada de $\sigma^2_{\hat{\beta}_1}$ es el error estándar del parámetro estimado, y
...
El p-valor del estadístico F puede calcularse utilizando la aproximación de su distribución para 
muestras grandes, tal que:
...
vs. $H_1 : \beta_j \neq 0$ para al menos una $j; j = 1, . . . , k$. Bajo la hipótesis nula, ninguna de las variables explicativas 
...
t.
    \item Un estadístico F elevado debería estar asociado con un aumento sustancial en el $R^2$. Si el término
...
restricciones implican a varios coeficientes. El estadístico F es extensible a este tipo de hipótesis conjuntas, 
y puede calcularse por cualquiera de los dos métodos que acabamos de mencionar. La mejor manera
...
La distinción entre variables de interés y variables de control puede ser establecida de forma matemáticamente 
precisa reemplazando el primer supuesto de mínimos cuadrados, es decir, el supuesto de esperanza 
condicional igual a cero, por un supuesto denominado independencia de la media condicional.
Consideremos una regresión con dos variables, en la cual $X_{1,i}$ es la variable de interés y $X_{2,i}$ es la variable 
de control. La independencia en media condicional requiere que la esperanza condicional de $u_i$ dados
...
está cerca de 1, entonces los regresores proporcionan buenas predicciones sobre la variable 
dependiente en esa muestra, en el sentido de que la varianza de los residuos MCO es
...
$$ES(\Delta\hat{Y}) = \frac{|\Delta\hat{Y}|}{\sqrt{F}}$$
...
explicativas de modo que, en la regresión transformada se contraste un único coeficiente con un estadísitco t.

En los modelos no lineales la función de regresión se interpreta mejor mediante su representación
gráfica y mediante el cálculo del efecto esperado sobre Y de la variación de una o más variables independientes
. La interpretación no suele ser la natural y habitual de los modelos lineales.
...
\begin{itemize}
    ...
    formas: utilizando datos en los que se observa la misma unidad observacional en diferentes momentos 
    del tiempo (datos de panel), utilizando la regresión de variables instrumentales, o utilizando
...
\begin{conceptoclave}
    ...
    \begin{enumerate}
        ...
        \item Utilizar un razonamiento a priori para identificar las fuentes potenciales más importantes 
        de sesgo de variable omitida, lo que lleva a una especificación base y a algunas
...
\begin{align*}
    ...
    &= Cov(X_i, u_i) - \beta_1 Cov(X_i, w_i) + Cov(w_i, u_i) - \beta_1 Cov(w_i, w_i) \\
    &= \underbrace{Cov(X_i, u_i)}_{=0 \text{ (Exogeneidad)}} - \underbrace{\beta_1 Cov(X_i, w_i)}_{=0 \text{ (Independencia de } w_i)} + \underbrace{Cov(w_i, u_i)}_{=0 \text{ (Independencia de } u_i)} - \beta_1 \underbrace{Cov(w_i, w_i)}_{=\sigma^2_w \text{ (Varianza)}} \\
    &= -\beta_1 \sigma^2_w
\end{align*}
...
\begin{conceptoclave}
    ...
    El sesgo por causalidad simultánea, asimismo denominado sesgo de ecuaciones simultáneas, 
    aparece en una regresión de Y sobre X, cuando, además del vínculo causal de 
    interés que va desde X hacia Y, existe un vínculo causal desde Y hacia X. Esta causalidad inversa 
    provoca que X esté correlacionado con el término de error en la regresión poblacional de interés.
\end{conceptoclave}
...
Este método de diferencias es útil cuando se disponen de datos para solo dos periodos. Sin embargo, 
cuando $T > 2$, es recomendable utilizar el método de efectos fijos, que permite analizar todas las
...
\paragraph{Algoritmo MCO en desviaciones respecto de su media} El estimador MCO de efectos fijos se
...
\section{Regresión con efectos fijos temporales}

La regresión con \textbf{efectos fijos temporales} permiten tener en cuenta las variables que son constantes
entre las entidades individuales, pero que evolucionan en el tiempo, como estándares de seguridad 
nacionales. El modelo seria:
...
distribución asintóticamente normal cuando n es grande. El homólogo para sección cruzada del Supuesto 2 sostiene 
que cada observación es independiente, lo que surge en virtud de un muestreo aleatorio simple. Por el contrario, el 
Supuesto 2 para datos de panel sostiene que las variables son independientes entre las distintas entidades individuales, 
pero no impone ninguna restricción de ese tipo dentro de una entidad individual. Por ejemplo, el Supuesto 2 permite que 
$X{it}$ esté correlacionada en el tiempo dentro de una entidad individual. Por ejemplo, en datos de series temporales, es común
...
\subsection*{Apéndice 10.2: Errores estándar de la regresión de efectos fijos}

\subsubsection*{La distribución asintótica del estimador de efectos fijos con n grande}

\paragraph{El estimador de efectos fijos} correspondiente a desviaciones respecto a la meda donde 
$\tilde{Y}_{it}=Y_{it}-\bar{Y}_{i},\tilde{X}_{it}=X_{it}-\bar{X}_{i},\bar{Y}_{i}=T^{-1}\Sigma_{i=1}^{T}Y_{it}\mathbf{y}\bar{X}_{i}=T^{-1}\Sigma_{i=1}^{T}X_{it}$ 
. La fórmula se obtiene sustituyendo la sumatoria simple por dos sumatorias, una sobre entidades y otra sobre periodos de tiempo tal que:
$$\widehat{\beta}_1=\frac{\sum_{i=1}^n\sum_{i=1}^T\widetilde{X}_{it}\widetilde{Y}_{it}}{\sum_{i=1}^n\sum_{t=1}^T\widetilde{X}_{it}^2}$$

Para la obtención de su distribución muestral se realiza de forma igual que en MCO común. Se sustituye $\tilde{Y}_{it}=\beta_1\tilde{X}_{it}+\tilde{u}_{it}$ en el 
numerador para obtener lo siguiente:
$$\widehat{\beta}_{1}=\beta_{1}+\frac{\frac{1}{nT}\sum_{i\operatorname{=}1}^{n}\sum_{t\operatorname{=}1}^{T}\tilde{X}_{it}\tilde{u}_{it}}{\frac{1}{nT}\sum_{i\operatorname{=}1}^{n}\sum_{t\operatorname{=}1}^{T}\tilde{X}_{it}^{2}}$$

Reorganizando la expresión y multiplicando ambos lados por $\sqrt{nT}$ se obtiene:
\begin{align*}
\sqrt{nT}(\hat{\beta}_{1}-\beta_{1})&=\frac{\sqrt{\frac{1}{n}}\sum_{i=1}^{n}\eta_{i}}{\hat{Q}_{\tilde{X}}}\\
\text{donde}\quad\eta_{i}=\sqrt{\frac{1}{T}}\sum_{t=1}^{T}\tilde{X}_{it}\tilde{u}_{it}\quad&\text{y}\quad\hat{Q}_{\tilde{X}}=\frac{1}{nT}\sum_{i=1}^{n}\sum_{t=1}^{T}\tilde{X}_{it}^{2}
\end{align*}

\paragraph{Distribución y errores estándar cuando n es grande} . Bajo los supuestos del concepto clave 10.3 (\ref{cc:10.3}), $\hat{Q}_{\tilde{X}}\xrightarrow{p}Q_{\tilde{X}}=ET^{-1}\sum_{t=1}^{T}\tilde{X}_{it}^{2}$ cuando 
$n\rightarrow\infty$. Además $\eta_i$ es i.i.d. para $i=1,\dots,n$ (por el supuesto 2) con media cero (por el supuesto 1) y varianza $\sigma_{\eta}^2$ que es finita (por el supuesto 3) 
, por lo que el teorema central del límite es aplicable $\sqrt{1/n}\sum_{i=1}^n\eta_i\xrightarrow{d}N(0,\sigma_\eta^2)$. Se obtiene entonce que:
$$\sqrt{nT}(\widehat{\beta}_1-\beta_1)\xrightarrow{d}-N\left(0,\frac{\sigma_\eta^2}{Q_{\tilde{X}}^2}\right)$$

De acuerdo a esto último la varianza de la distribución de muestras grandes de $\hat{\beta}_1$ es:
$$\mathrm{var}(\widehat{\beta}_1)=\frac{1}{nT}\frac{\sigma_\eta^2}{Q_{\tilde{X}}^2}$$

La fórmula de los errores estándar agrupados sustituye los momento poblacionales de esta ecuación por sus sustitutos muestrales:
\begin{align*}
ES(\hat{\beta}_{1})&=\sqrt{\frac{1}{nT}\frac{s_{\eta}^{2}}{\hat{Q}_{\tilde{x}}^{2}}} \\
\text{donde}\quad s_{\eta}^{2}&=\frac{1}{n-1}\sum_{i=1}^{n}(\hat{\eta}_{\mathrm{i}}-\bar{\hat{\eta}})^{2}=\frac{1}{n-1}\sum_{i=1}^{n}\hat{\eta}_{i}^{2}\quad(\text{errores estandar agrupados})
\end{align*}

Donde $\hat{\eta}_{i}=\sqrt{1/T}\Sigma_{t=1}^{T}\tilde{X}_{it}\hat{u}_{it}$ es el homólogo muestral de $\eta_i$, 
y $\bar{\hat{\eta}}=(1/n)\sum_{i=1}^n\hat{\eta}_i$. La última igualdad ocurre porque $\bar{\hat{\eta}}=0$, que se desprende 
de que los residuos y los regresores no estan correlacionados. Téngase en cuenta que $s^n_{\hat{\eta}}$ es un estimador de la varianza muestral de 
$\hat{\eta}_i$, este es un estimador consistente de $\sigma_{\eta}^2$ a medida que $n\rightarrow\infty$ en presencia de heterocedásticidad o 
autocorrelación, debido a esto el estadísitco t tiene una distribución normal estándar bajo la hipótesis nula cuando $n\rightarrow\infty$.

Esto es aplicable a regresión múltiple y el estadistico F para el contraste de q restricciones se distribuye como $F_{q,\infty}$.

\paragraph{¿Por qué no es válido el estimaodr habitual heterocedástico-robusto para los datos de panel?}
. Existen 2 razones, la primera es que no permite la correlación serial dentro de un grupo (cluster). La varianza $\eta_i$ puede escribirse como 
la suma de las varianzas más las covarianzas. Sea $\tilde{v}_{it}=\tilde{X}_{it}\tilde{u}_{it}$; Entonces:
\begin{align*}
\mathrm{var}(\eta_{i})=\mathrm{var}\left(\sqrt{\frac{1}{T}}\sum_{t=1}^{T}\widetilde{\nu}_{it}\right)=\frac{1}{T}\mathrm{var}\left(\widetilde{\nu}_{i1}+\widetilde{\nu}_{i2}+\cdots+\widetilde{\nu}_{iT}\right) \\
=\frac{1}{T}\mathrm{var}(\widetilde{\nu}_{i1})+\mathrm{var}(\widetilde{\nu}_{i2})+\cdot\cdot+\mathrm{var}(\widetilde{\nu}_{iT}) \\
+2\mathrm{cov}(\widetilde{\nu}_{i1},\widetilde{\nu}_{i2})+\cdots+2\mathrm{cov}(\widetilde{\nu}_{iT-1},\widetilde{\nu}_{iT})].
\end{align*}

La fórmula del capítulo 5 obvia todas las covarianzas de la parte final, por lo que si existe correlación serial es estimador 
heterocedástico-robusto es inconsistente.

La segunda razón es que si $T$ es pequeño, la estimación de los efectos fijos introduce sesgo en el estimador heterocedástico-robusto. 
El único caso en el que pueden utilizarse los errores heterocedástico-robustos habituales con datos de panel es con la regresión e efectos 
fijos con $T=2$, siendo equivalentes los errores heterocedástico-robustos y los estándar agrupados.
...
valor esperado de Y dados los regresores.
...
Los coeficientes del modelo logit se estiman mediante el método de máxima verosimilitud, que es consistente 
y normalmente distribuido en muestras grandes, permitiendo la construcción de estadísticos (t) e
...
suponemos independientes es su productorio), considerada como una función de los coeficientes desconocidos. 
Para n observaciones independientes, la función de verosimilitud es:
...
\begin{align*}
\Pr(Y_1=y_1,Y_2=y_2,\ldots,Y_n=y_n) & =\left[p^{y_1}(1-p)^{\left(1-y_1\right)}\right] \times\left[p^{y_2}(1-p)^{\left(1-y_2\right)}\right] \times \cdots \times\left[p^{y_n}(1-p)^{\left(1-y_n\right)}\right] \\
& =p^{\left(y_1+\cdots+y_n\right)}(1-p)^{n-\left(y_1+\cdots+y_n\right)} \\
...
\begin{align*}
\operatorname{Pr}\left(Y_1\right. & \left.=y_1, \ldots, Y_n=y_n \mid X_{1 i}, \ldots, X_{k i}, i=1, \ldots, n\right) \\
& =\operatorname{Pr}\left(Y_1=y_1 \mid X_{11}, \ldots, X_{k 1}\right) \times \ldots \times \operatorname{Pr}\left(Y_n=y_n \mid X_{1 n}, \ldots, X_{k n}\right) \\
& =p_1^{y_1}\left(1-p_1\right)^{1-y_1} \times \ldots \times p_n^{y_n}\left(1-p_n\right)^{1-y_n} .
\end{align*}
...
\begin{align*}
\ln [f_{probit} &(\beta_0 +\dots + \beta_k ; Y_1, \dots , Y_n \mid X_{1i}, \dots , X_{ki}), i=1, \dots , n] = \\
 &\sum_{i=1}^n [Y_i\ln(\Phi(\beta_0+\beta_1X_{1i}+\cdots+\beta_kX_{ki}))+ \\ 
 &(1-Y_i) \ln(1-\Phi(\beta_0+\beta_1 X_{1i}+\cdots+\beta_k X_{ki}))]
\end{align*}
...
probit, la probabilidad estimada $\hat{p}(x)$ dado un valor fijo del regresor x es $\hat{p}(x) = \Phi( \hat{\beta}_0 + \hat{\beta}_1 x)$
Donde $\hat{\beta}_0^{EMV}$ y $\hat{\beta}_1^{EMV}$ son los EMV de los 2 coeficientes probit. La varianza de la distribución muestral de
esta probabilidad estimada se obtiene mediante una expansión de Taylor de primer orden:
$$\hat{p}(x)=\Phi\left(\hat{\beta}_0^{E M V}+\hat{\beta}_1^{E M V} x\right) \cong c+a_0\left(\hat{\beta}_0^{E M V}-\beta_0\right)+a_1\left(\hat{\beta}_1^{E M V}-\beta_1\right)$$

donde $a_0$ y $a_1$ son las derivadas parciales y $c$ una constante donde $c=\Phi(\hat{\beta}_0^{E M V})$ y $a_0=\frac{\partial \Phi (\beta_0 + \beta_1x)}{\partial \beta_0}\mid \hat{\beta}_0^{E M V},\hat{\beta}_1^{E M V}$
y $a_1=\frac{\partial \Phi (\beta_0 + \beta_1x)}{\partial \beta_1}\mid \hat{\beta}_0^{E M V},\hat{\beta}_1^{E M V}$. La varianza de $\hat{p}(x)$ es:
\begin{align*}
\operatorname{var}[\hat{p}(x)] & \cong \operatorname{var}\left[c+a_0\left(\hat{\beta}_0^{E M V}-\beta_0\right)+a_1\left(\hat{\beta}_1^{E M V}-\beta_1\right)\right]= \\
& =a_0^2 \operatorname{var}\left(\hat{\beta}_0^{E M V}\right)+a_1^2 \operatorname{var}\left(\hat{\beta}_1^{E M V}\right)+2 a_0 a_1 \operatorname{cov}\left(\hat{\beta}_0^{E M V}, \hat{\beta}_1^{E M V}\right)
\end{align*}
...
\paragraph{Condiciones para un intrumento válido:} Una variable instrumental válida debe cumplir dos condiciones:
...
Los coeficientes deben estar exactamente identificados o sobreidentificados para ser estimados
...
\paragraph{Relevancia y Exogeneidad de los Instrumentos} Las condiciones de relevancia y exogeneidad de
...
Una regla práctica sencilla es que no es necesario preocuparse de los instrumentos débiles si el 
estadístico F de la primera etapa es mayor que 10. (Para más detalles, véase el Apéndice 12.5). Esto se
...
Sin embargo, si los coeficientes están exactamente identificados, no se pueden descartar los instrumentos 
débiles. Aun cuando los coeficientes estén sobreidentificados, puede que no se disponga de suficientes
...
\begin{enumerate}
    \item \textbf{Encontrar instrumentos adicionales, fuertes:} Esto requiere un conocimiento profundo del
problema en cuestión y puede implicar el rediseño del conjunto de datos y de la naturaleza del
estudio empírico.
    \item \textbf{Continuar el análisis empírico con los instrumentos débiles:} Empleando métodos distintos
...
\begin{enumerate}
    \item \textbf{Relevancia de los instrumentos} El papel de la condición de relevancia de los instrumentos en
    ...
    distribución normal es una buena aproximación para muestras grandes, pero no necesariamente para
...
conocimiento personal de los problemas empíricos que se están analizando.
    \item \textbf{Sobreidentificados:} Si los coeficientes están sobreidentificados, es posible contrastar la 
    sobreidentificación de las restricciones, es decir, contrastar la hipótesis de que los instrumentos extras son
...
\paragraph{Contraste de Sobreidentificación de Restricciones} Supongamos que se dispone de un único 
regresor endógeno y de dos instrumentos. Se pueden calcular dos estimadores MC2E diferentes: uno que
...
\begin{conceptoclave}
    ...
    muestras grandes J se distribuye $\chi^2_{m-k}$, donde $m-k$ es el «grado de sobreidentificación»
    , es decir, el número de instrumentos menos el número de regresores endógenos.
\end{conceptoclave}
...
el estadístico J de contraste de sobreidentificación es exactamente igual a cero (sería una $\chi^2_{m-k=0}$).
...
Como se discutió en el Capítulo 8, la variación en los efectos causales que dependen de variables observables 
puede estimarse incluyendo funciones no lineales apropiadas o interacciones con $X_i$. Por ejemplo,
si $W_{1i}$ es un indicador binario que representa el género, los efectos causales diferentes para hombres y
...
ausencia de aleatoriedad, el incumplimiento del protocolo de tratamiento, la deserción, los efectos experimentales 
y los tamaños muestrales pequeños.
...
el estimador MCO debido a la correlación entre $X_i$ y $u_i$ para los que permanecen en la muestra.
...
sujetos como los instructores saben quién está en el grupo de tratamiento.
    \item \textbf{Muestras pequeñas:}Un tamaño de muestra pequeño no sesga los estimadores, pero puede llevar a estimaciones imprecisas 
    del efecto causal. También plantea problemas para la validez de los intervalos de confianza y
...
Las amenazas a la validez externa comprometen la capacidad de generalizar los resultados a otras poblaciones 
y escenarios. Las principales amenazas incluyen:
\begin{enumerate}
...
con exreclusos podría no ser generalizable a trabajadores que nunca han cometido un delito.
    \item \textbf{Programa o política no representativa:}La política o programa de interés debe ser suficientemente 
    similar al programa estudiado para permitir la generalización de los resultados. La
...
cambiar el entorno económico, afectando la generalización de los resultados. Por ejemplo, un programa 
de formación laboral podría complementar la capacitación proporcionada por empleadores
...
Mínimos Cuadrados Ordinarios (MCO) suele funcionar bien bajo ciertos supuestos (\ref{cc:6.4}). Sin
...
Los conjuntos de datos pueden considerarse grandes no solo por el número de observaciones, sino también 
por la cantidad de predictores, o incluso ambos. Los conjuntos de datos puede no ser estándar,
...
El manejo de grandes volúmenes de datos presenta varios desafíos computacionales, como el almacenamiento 
y el acceso eficiente a la información. Además, la estimación de modelos complejos requiere
...
En el modelo de regresión estandarizado de la Ecuación (\ref{eq:14.2}), la predicción para el valor fuera de la
...
\begin{equation}
Y_{oos}-(\hat{\beta}_{1}X_{oos,1}+\cdots+\hat{\beta}_{k}X_{oos,k})=u_{oos}-[(\hat{\beta}_{1}-\beta_{1})X_{oos,1}+\cdots+(\hat{\beta}_{k}-\beta_{k})X_{oos,k}]
\label{eq:14.3}
\end{equation}

Donde la expresión final se obtiene usando la Ecuación (\ref{eq:14.2}) y $u_{oos}$ es el valor del error $u$ para la
observación fuera de la muestra. Debido a que $u_{oos}$ es independiente de los datos usados para estimar
los coeficientes y no está correlacionado con $X_{oos}$, el MSPE en la Ecuación (\ref{eq:14.1}) para el modelo de
...
En la práctica, las medias y desviaciones estándar poblacionales de las variables originales no son conocidas. 
Por lo tanto, se utilizan las medias y varianzas de la muestra para estandarizar los regresores, y
...
$Y_n$ para cada una de las $n_{test}$ observaciones en la submuestra de prueba. El MSPE se estima con los
...
\begin{conceptoclave}
    ...
    \begin{enumerate}
        ...
        de la submuestra 1 y la ecuación (\ref{eq:14.5}). Lo llamaremos $\widehat{MSPE}_1$.
...
Los componentes principales de un conjunto de variables estandarizadas X son combinaciones lineales
...
Donde $\Delta\mathrm{Inf}_t$ representa la variación en la tasa de inflación del trimestre $t$ respecto al trimestre $t - 1$.
...
Donde el estadístico t para la variable $\mathrm{Desemp}_{t-1}$ es -2.23, lo que indica significancia al nivel del
...
adicional. Los modelos ARD de las Ecuaciones (\ref{eq:14.16}) y (\ref{eq:14.17}) se denominan ARD(4,1) y ARD(4,4)
respectivamente.

\begin{conceptoclave}
    {Concepto clave 14.4: El modelo autorregresivo de retardos distribuidos}
    El modelo autorregresivo de retardos distribuidos con p retardos de $Y_t$ y q retardos de $X_t$,
...
En el concepto clave 14.6 (\ref{cc:14.6}), se asumió que tanto la variable dependiente como los regresores eran estacionarios. 
Sin embargo, si estas variables no son estacionarias, los contrastes de hipótesis, los intervalos de
...
\begin{itemize}
    \item \textbf{Sesgo hacia cero en coeficientes autorregresivos:} El estimador MCO del coeficiente autorregresivo 
    en un modelo AR(1) está sesgado hacia cero si el verdadero valor es 1.
    \item \textbf{Distribuciones no normales del estadístico t:} El estadístico t MCO puede tener una distribución 
    distinta de la normal, afectando la validez de los intervalos de confianza y los contrastes de
hipótesis.
...
\begin{conceptoclave}
    ...
    Donde $\alpha$ es un coeficiente desconocido y el estadístico ADF es el estadístico t MCO para
...
\begin{equation*}
\begin{gathered}
\widehat{\% V P_t}=-\underset{(0,23)}{0,65}+\underset{(0,14)}{0,47 I H_t}+\underset{(0,08)}{0,14 I H_{t-1}}+\underset{(0,06)}{0,06 I H_{t-2}} \\
+\underset{(0,05)}{0,07 I H_{t-3}}+\underset{(0,03)}{0,03 I H_{t-4}}+\underset{(0,03)}{0,05 I H_{t-5}}+\underset{(0,04)}{0,05 I H_{t-6}}
\end{gathered}
\end{equation*}
...
\paragraph{Efectos dinámicos y modelo de retardos distribuidos} Debido a que los efectos dinámicos necesariamente 
ocurren en el tiempo, resulta necesario que el modelo econométrico utilizado para estimar los
efectos causales dinámicos incorpore retardos. Para hacerlo, se puede expresar $Y_t$ como un modelo de
retardos distribuidos.
\begin{equation}
    Y_t=\beta_0+\beta_1X_t+\beta_2X_{t-1}+\beta_3X_{t-2}+\dots+\beta_{r+1}X_{t-r}+u_t
\label{eq:15.3}
\end{equation}
...
\begin{conceptoclave}
    ...
    existen dos tipos diferentes de exogeneidad, es decir, dos condiciones diferentes de exogeneidad:
...
La autocorrelación en $u_t$ no afecta la consistencia de MCO ni introduce sesgo, pero los errores estándar
...
supuestos del Concepto clave 15.2(\ref{cc:15.2}).
\begin{align*}
...
Si $v_t$ es i.i.d. entonces $var(\bar{v})=var(v_t)/T$ y es aplicable la fórmula de $\hat{\beta}_1$ del concepto clave 4.4
...
y $var(\bar{v})\neq var(v_t)/T$, en ese caso la varianza de $\bar{v}$ es:
\begin{align*}
    \mathrm{var}(\bar{v}) & =\mathrm{var}[(v_{1}+v_{2}+\cdots+v_{T})/T] \\
...
La primera estrategia para estimar los multiplicadores dinámicos consiste en utilizar MCO para los coeficientes 
del modelo ARD de la Ecuación (\ref{eq:15.21}). La deducción que conduce a dicha ecuación demuestra
...
La segunda estrategia para estimar los multiplicadores dinámicos cuando $X_t$ es estrictamente exógena
...
(\ref{eq:15.23}), calculados mediante la regresión de $\hat{\tilde{Y}}_t$ sobre $\hat{\tilde{X}}_t$ y $\hat{\tilde{X}}_{t-1}$ (con un término independiente), donde:
$$\hat{\tilde{X}}_t=X_t-\hat{\phi}_1X_{t-1}\quad\mathrm{y}\quad\hat{\tilde{Y}}_t=Y_t-\hat{\phi}_1Y_{t-1}.$$
...
\begin{itemize}
    \item \textbf{Ventaja de MCO (ARD):} Proporciona una representación compacta de la distribución de retardos, 
    reduciendo el número de parámetros a estimar. Esto es útil cuando la distribución de retardos
es larga y compleja.
